    ...
    this.dataService.hierarchy
      ...
      .subscribe(data => {
        ...
        const transformerEnter = transformer.enter().append('g')
          ...
          .on('mousedown', function(d) {
            const analytic = d3.event.ctrlKey ? d.transformer.clone() : d.transformer;
            const mousedownCallback = () => {
              d.mousedownHandler = undefined;
              component.dragService.startDrag({sourceElement: this as SVGGraphicsElement, object: analytic, offset: {x: -transformerWidth/2, y: -channelSpacing}});
            };
            d.mousedownHandler = {
              callback: mousedownCallback,
              timeout: setTimeout(mousedownCallback, 250)
